...
// *** Begin Original Porter Stermmer ***
...
// *** End Original Porter Stemmer ***

#include <string>
#include "util/string/porter_stemmer.h"

PorterStemmer::PorterStemmer() {
  stemmer_ = create_stemmer();
}

PorterStemmer::~PorterStemmer() {
  free_stemmer(stemmer_);
}

std::string PorterStemmer::StemTerm(const std::string& term) const {
  std::string buffer(term);
  if (!term.empty())
    buffer.resize(stem(stemmer_, &buffer[0], term.size()-1) + 1);
  return buffer;
}
