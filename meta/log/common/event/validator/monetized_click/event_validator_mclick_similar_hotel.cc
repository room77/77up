...
class EventMClickSimilarHotelValidator : public ValidatorForType<tEventMClickSimilarHotel> {
 protected:
  virtual uint8_t HandleValidation(const tLogElement& element,
                                   const tEventMClickSimilarHotel& event) const {
    ...
    if (!(CanonicalObjectsValidator::Validate(element, event.linked_from))) return 0;
    if (event.num_additional_rates < 0) return 0;
    if (event.range_min < 0) return 0;
    if (event.range_max < 0) return 0;
    if (event.position_similar < 0) return 0;

    return event::ValidatorBase::kEventDataBit;
  }
};

INIT_ADD_REQUIRED("event_mclick_similar_hotel_validator", [] {
    // register the validator class to the ValidatorBase factory
    ValidatorBase::bind(pair<string, string> ("Monetized Click", "Similar Hotel"),
                        [] { return new EventMClickSimilarHotelValidator(); });
  });
